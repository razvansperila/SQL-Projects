...
#labeling pokemons as fighter or defender based on their stats. if there is a tie, compare hp. if hp over median value then labled as browler, if under labled as bench

select `name`,
	case
		when att > def then 'Fighter'
        when def > att then 'Defender'
        else if(hp > (select (max(hp) + min(hp)) / 2 from pokemon_project_staging), 'Browler', 'Bench')
	end as label
from(
		select `name`, hp,
